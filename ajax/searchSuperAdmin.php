...
$query = "SELECT * FROM user
          WHERE nama LIKE '%$keyword%' AND level = 'Super Admin'
          OR email LIKE '%$keyword%' AND level = 'Super Admin'
          ORDER BY nama ASC";

$golongan = mysqli_query($conn, "SELECT user.*, jabatan.nama_jabatan, golongan.nama_golongan FROM user INNER JOIN jabatan ON user.kode_jabatan = jabatan.kode_jabatan INNER JOIN golongan ON user.kode_golongan=golongan.kode_golongan WHERE user.level = 'Super Admin' ORDER BY user.nama ASC");
...
<table class="table-3">
  ...
  <tbody>
    ...
    <?php if (mysqli_num_rows($golongan) <= 0) { ?>
      <tr>
        <td colspan="6" style="text-align: left;">Maaf, belum ada data apapun disini.</td>
      </tr>
    <?php } else { ?>
      <?php if (mysqli_num_rows($data) <= 0) { ?>
        <tr>
          <td colspan="6" style="text-align: left;">Maaf kami tidak menemukan admin dengan nama <strong style="text-transform: capitalize;">'<?= $keyword; ?>'</strong>.</td>
        </tr>
      <?php } ?>
    <?php } ?>
  </tbody>
